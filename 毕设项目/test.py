...
def find(image, contours, hierachy, root=0):
    ...
    print(contours)
    print(rec)
    cutrect = image[238:550, 1301:1613]
    # cv2.drawContours(image, contours, rec[0][6], (0, 0, 255), 2)
    # cv2.drawContours(image, contours, rec[0][6], (0, 0, 255), 2)
    # cv2.drawContours(image, contours, rec[1][6], (0, 0, 255), 2)
    # cv2.drawContours(image, contours, rec[3][6], (0, 0, 255), 2)
    # cv2.drawContours(image, contours, rec[i][6], (0, 0, 255), 2)
    # cv2.drawContours(image, contours, rec[j][6], (0, 0, 255), 2)
    # cv2.drawContours(image, contours, rec[k][6], (0, 0, 255), 2)
    cv2.namedWindow("Image", cv2.WINDOW_NORMAL)
    cv2.imshow('Image', result)
    # cv2.imwrite('result.png',cutrect)
    # cv2.imshow('Image', cutrect)
    cv2.waitKey(0)
